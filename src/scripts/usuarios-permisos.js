...
function createNewUserModal() {
  ...
  root.querySelectorAll('[name="newUserRole"]').forEach((input) => input.addEventListener('change', updateDataFieldsForRole)); confirmPinField.addEventListener('input', () => validatePins(true)); pinField.addEventListener('input', () => validatePins(false)); emailField.addEventListener('input', validateEmail); emailField.addEventListener('blur', validateEmail); renderLocalOptions(); updateDataFieldsForRole();
