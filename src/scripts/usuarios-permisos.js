...
function createNewUserModal() {
  ...
  const validatePins = (shake = false, requireConfirmation = false) => { const pin = pinField.value.trim(); const confirmPin = confirmPinField.value.trim(); const invalid = Boolean(confirmPin) && pin !== confirmPin; setFieldError(confirmPinField, invalid); pinFeedback.classList.toggle('hidden', !invalid); if (invalid && shake) { const container = confirmPinField.closest('div'); container.classList.remove('animate-field-shake'); void container.offsetWidth; container.classList.add('animate-field-shake'); } return !invalid && (!requireConfirmation || Boolean(confirmPin)); };
  ...
  root.querySelectorAll('[name="newUserRole"]').forEach((input) => input.addEventListener('change', updateDataFieldsForRole)); confirmPinField.addEventListener('input', () => validatePins(false)); confirmPinField.addEventListener('blur', () => validatePins(true)); pinField.addEventListener('input', () => validatePins(false)); emailField.addEventListener('input', validateEmail); emailField.addEventListener('blur', validateEmail); renderLocalOptions(); updateDataFieldsForRole();
  next.addEventListener('click', () => { if (step === 1) { setStep(2); return; } const role = selectedRole(); const active = root.querySelector('#fieldNewActivo').value === 'SI'; const name = root.querySelector('#fieldNewNombreCompleto').value.trim(); const pin = pinField.value.trim(); const login = root.querySelector('#fieldNewUsuarioLogin').value.trim(); const local = selectedLocals(); const pinsValid = validatePins(true, true); const emailValid = validateEmail(); if (!name || !pin || !pinsValid || !emailValid || ((role === 'Administrador' || role === 'Supervisor') && !login) || (role !== 'Administrador' && !local.length)) { feedback.textContent = !pinsValid ? 'Confirma el PIN y revisa que ambos campos coincidan antes de continuar.' : !emailValid ? 'Ingresa un correo electrónico válido antes de continuar.' : 'Completa los campos obligatorios marcados con * antes de continuar.'; feedback.classList.remove('hidden'); return; } feedback.classList.add('hidden'); root.querySelector('#newUserSummary').textContent = `${name} · ${role} · ${local.join(', ') || 'Todos los locales'} · ${active ? 'Activo' : 'Inactivo'}`; setStep(3); });
  back.addEventListener('click', () => setStep(step - 1));
  form.addEventListener('submit', async (event) => { event.preventDefault(); const pin = pinField.value.trim(); const pinsValid = validatePins(true, true); const emailValid = validateEmail(); if (!pinsValid || !emailValid) { feedback.textContent = !pinsValid ? 'Confirma el PIN y revisa que ambos campos coincidan antes de crear el usuario.' : 'Ingresa un correo electrónico válido.'; feedback.classList.remove('hidden'); return; }
