...
function createNewUserModal() {
  ...
  root.innerHTML = `
    ...
    <div class="relative z-[1] max-h-[92vh] w-full max-w-[900px] overflow-auto rounded-[32px] border border-neutral-charcoal/12 bg-[#fff8ee] p-xl shadow-brand md:p-2xl">
      <div class="flex items-start justify-between gap-lg"><div><p class="text-xs font-black uppercase tracking-[0.18em] text-neutral-muted">Nuevo usuario</p><h2 id="newUserTitle" class="mt-sm text-[clamp(30px,4vw,46px)] font-black tracking-[-0.05em]">Crear acceso para Administrador</h2><p class="mt-sm text-sm font-bold text-neutral-muted">Completa el alta en tres pasos claros.</p></div><button type="button" data-close class="grid size-11 place-items-center rounded-2xl border border-neutral-charcoal/10 bg-white text-xl font-black">×</button></div>
  ...
  root.querySelectorAll('[name="newUserRole"]').forEach((input) => input.addEventListener('change', updateDataFieldsForRole)); confirmPinField.addEventListener('input', () => validatePins()); confirmPinField.addEventListener('blur', () => validatePins(true, false, true)); pinField.addEventListener('input', () => validatePins()); emailField.addEventListener('input', validateEmail); emailField.addEventListener('blur', validateEmail); renderLocalOptions(); updateDataFieldsForRole();
  next.addEventListener('click', () => { if (step === 1) { setStep(2); return; } const role = selectedRole(); const active = root.querySelector('#fieldNewActivo').value === 'SI'; const name = root.querySelector('#fieldNewNombreCompleto').value.trim(); const pin = pinField.value.trim(); const login = root.querySelector('#fieldNewUsuarioLogin').value.trim(); const local = selectedLocals(); const pinsValid = validatePins(true, true, true); const emailValid = validateEmail(); if (!name || !pin || !pinsValid || !emailValid || ((role === 'Administrador' || role === 'Supervisor') && !login) || (role !== 'Administrador' && !local.length)) { feedback.textContent = !pinsValid ? 'Confirma el PIN y revisa que ambos campos coincidan antes de continuar.' : !emailValid ? 'Ingresa un correo electrónico válido antes de continuar.' : 'Completa los campos obligatorios marcados con * antes de continuar.'; feedback.classList.remove('hidden'); return; } feedback.classList.add('hidden'); root.querySelector('#newUserSummary').textContent = `${name} · ${role} · ${local.join(', ') || 'Todos los locales'} · ${active ? 'Activo' : 'Inactivo'}`; setStep(3); });
