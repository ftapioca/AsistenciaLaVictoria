...
function createInputGroup(id, label, disabled = false, type = 'text', help = '') {
  return `
    <label class="grid gap-sm">
      ...
      ${help ? `<span class="text-xs font-bold leading-5 text-neutral-muted">${help}</span>` : ''}
    </label>
  `;
}
...
function createNewUserModal() {
  ...
  root.innerHTML = `
    ...
    <div class="relative z-[1] max-h-[92vh] w-full max-w-[900px] overflow-auto rounded-[32px] border border-neutral-charcoal/12 bg-[#fff8ee] p-xl shadow-brand md:p-2xl">
      <div class="flex items-start justify-between gap-lg"><div><p class="text-xs font-black uppercase tracking-[0.18em] text-neutral-muted">Nuevo usuario</p><h2 id="newUserTitle" class="mt-sm text-[clamp(30px,4vw,46px)] font-black tracking-[-0.05em]">Crear nuevo usuario</h2><p class="mt-sm text-sm font-bold text-neutral-muted">Completa el alta en tres pasos claros.</p></div><button type="button" data-close class="grid size-11 place-items-center rounded-2xl border border-neutral-charcoal/10 bg-white text-xl font-black">×</button></div>
      ...
      <form class="mt-xl grid gap-lg">
        ...
        <section data-step="2" class="hidden grid gap-lg"><div class="rounded-3xl border border-neutral-charcoal/10 bg-white/72 p-lg text-sm font-bold text-neutral-muted">Los campos con <strong class="text-brand-ketchup">*</strong> son obligatorios para crear el acceso.</div><div class="grid gap-lg md:grid-cols-2">${createInputGroup('newNombreCompleto', 'Nombre completo <strong class="text-brand-ketchup">*</strong>', false, 'text', 'Nombre con el que se identificará al usuario en la operación.')}<div data-new-user-login>${createInputGroup('newUsuarioLogin', 'Usuario login <strong class="text-brand-ketchup">*</strong>', false, 'text', 'Se usa para iniciar sesión como administrador o supervisor.')}</div><div data-new-user-local class="md:col-span-2"><div class="grid gap-sm"><span class="text-sm font-black uppercase tracking-[0.16em] text-neutral-muted">Locales asignados <strong class="text-brand-ketchup">*</strong></span><span class="text-xs font-bold leading-5 text-neutral-muted">Selecciona todos los locales donde podrá operar.</span><div id="newUserLocals" class="mt-sm grid gap-sm sm:grid-cols-2"></div></div></div>${createInputGroup('newCargo', 'Cargo', false, 'text', 'Describe su función operativa; es opcional.')}${createSelectGroup('newActivo', 'Activo', ['SI', 'No'], 'h-[52px] min-h-0 py-0', 'Define si el acceso quedará habilitado al crearlo.')} ${createInputGroup('createPin', 'PIN <strong class="text-brand-ketchup">*</strong>', false, 'password', 'Clave personal de acceso.')}<div>${createInputGroup('createConfirmPin', 'Confirmar PIN <strong class="text-brand-ketchup">*</strong>', false, 'password', 'Repítelo para evitar crear el acceso con una clave equivocada.')}<p id="newUserPinFeedback" class="mt-sm hidden text-xs font-black text-brand-ketchup">Los PIN no coinciden. Revisa ambos campos.</p><p id="newUserPinSuccess" class="mt-sm hidden text-xs font-black text-brand-lettuce">Los PIN coinciden correctamente.</p></div><div>${createInputGroup('newEmail', 'Email', false, 'email', 'Dato de contacto opcional.')}<p id="newUserEmailFeedback" class="mt-sm hidden text-xs font-black text-brand-ketchup">Ingresa un correo electrónico válido.</p></div>${createInputGroup('newTelefono', 'Teléfono', false, 'tel', 'Dato de contacto opcional.')}</div></section>
  ...
  const setStep = (value) => { const previousStep = step; step = value; root.querySelectorAll('[data-step]').forEach((node) => node.classList.toggle('hidden', Number(node.dataset.step) !== step)); root.querySelectorAll('[data-step-indicator]').forEach((node) => { const active = Number(node.dataset.stepIndicator) === step; node.className = `rounded-xl px-sm py-sm ${active ? 'bg-brand-bun text-neutral-charcoal' : 'bg-neutral-charcoal/8 text-neutral-muted'}`; }); if (step === 2 && previousStep === 1) clearInlineValidation(); root.querySelector('#newUserTitle').textContent = step === 2 ? `Creando usuario tipo ${selectedRole()}` : 'Crear nuevo usuario'; back.classList.toggle('hidden', step === 1); next.classList.toggle('hidden', step === 3); submit.classList.toggle('hidden', step !== 3); };
  ...
  const pinField = root.querySelector('#fieldCreatePin'); const confirmPinField = root.querySelector('#fieldCreateConfirmPin'); const pinFeedback = root.querySelector('#newUserPinFeedback'); const pinSuccess = root.querySelector('#newUserPinSuccess'); const emailField = root.querySelector('#fieldNewEmail'); const emailFeedback = root.querySelector('#newUserEmailFeedback');
  const setFieldError = (field, invalid) => { field.classList.toggle('border-brand-ketchup', invalid); field.classList.toggle('bg-brand-ketchup/10', invalid); field.classList.toggle('pin-field-alert', invalid); };
  const setRequiredAlert = (field, invalid) => { field.classList.toggle('required-field-alert', invalid); field.classList.remove('animate-required-alert'); if (invalid) { void field.offsetWidth; field.classList.add('animate-required-alert'); } };
  const clearRequiredAlert = (field) => setRequiredAlert(field, false);
  const clearInlineValidation = () => { root.querySelectorAll('.required-field-alert, .pin-field-alert, .pin-field-success, .animate-required-alert, .animate-pin-alert').forEach((field) => field.classList.remove('required-field-alert', 'pin-field-alert', 'pin-field-success', 'animate-required-alert', 'animate-pin-alert', 'border-brand-ketchup', 'bg-brand-ketchup/10')); pinFeedback.classList.add('hidden'); pinSuccess.classList.add('hidden'); emailFeedback.classList.add('hidden'); feedback.classList.add('hidden'); };
  const validatePins = (shake = false, requireConfirmation = false, showFeedback = false) => { const pin = pinField.value.trim(); const confirmPin = confirmPinField.value.trim(); const invalid = Boolean(confirmPin) && pin !== confirmPin; const matches = Boolean(pin && confirmPin) && pin === confirmPin; setFieldError(confirmPinField, invalid && showFeedback); confirmPinField.classList.toggle('pin-field-success', matches); pinFeedback.classList.toggle('hidden', !invalid || !showFeedback); pinSuccess.classList.toggle('hidden', !matches); confirmPinField.classList.remove('animate-pin-alert'); if (invalid && shake && showFeedback) { void confirmPinField.offsetWidth; confirmPinField.classList.add('animate-pin-alert'); } return !invalid && (!requireConfirmation || Boolean(confirmPin)); };
  const validateEmail = () => { const invalid = Boolean(emailField.value.trim()) && !emailField.validity.valid; setFieldError(emailField, invalid); emailFeedback.classList.toggle('hidden', !invalid); return !invalid; };
  ...
  const validateRequiredFields = () => { const role = selectedRole(); const requiresLogin = role === 'Administrador' || role === 'Supervisor'; const requiresLocal = role !== 'Administrador'; const localRoot = root.querySelector('#newUserLocals'); const nameMissing = !root.querySelector('#fieldNewNombreCompleto').value.trim(); const loginMissing = requiresLogin && !root.querySelector('#fieldNewUsuarioLogin').value.trim(); const pinMissing = !pinField.value.trim(); const confirmationMissing = !confirmPinField.value.trim(); const localMissing = requiresLocal && !selectedLocals().length; setRequiredAlert(root.querySelector('#fieldNewNombreCompleto'), nameMissing); setRequiredAlert(root.querySelector('#fieldNewUsuarioLogin'), loginMissing); setRequiredAlert(pinField, pinMissing); setRequiredAlert(confirmPinField, confirmationMissing); setRequiredAlert(localRoot, localMissing); return !nameMissing && !loginMissing && !pinMissing && !confirmationMissing && !localMissing; };
  const close = () => { root.classList.add('hidden'); root.classList.remove('grid'); form.reset(); clearInlineValidation(); root.querySelector('#fieldNewActivo').value = 'SI'; renderLocalOptions(); updateDataFieldsForRole(); setStep(1); };
  root.querySelectorAll('[data-close], [data-backdrop]').forEach((node) => node.addEventListener('click', close));
  root.querySelectorAll('[name="newUserRole"]').forEach((input) => input.addEventListener('change', updateDataFieldsForRole)); root.querySelector('#fieldNewNombreCompleto').addEventListener('input', (event) => clearRequiredAlert(event.currentTarget)); root.querySelector('#fieldNewUsuarioLogin').addEventListener('input', (event) => clearRequiredAlert(event.currentTarget)); root.querySelector('#newUserLocals').addEventListener('change', () => clearRequiredAlert(root.querySelector('#newUserLocals'))); confirmPinField.addEventListener('input', () => { clearRequiredAlert(confirmPinField); validatePins(); }); confirmPinField.addEventListener('blur', () => validatePins(true, false, true)); pinField.addEventListener('input', () => { clearRequiredAlert(pinField); validatePins(); }); emailField.addEventListener('input', validateEmail); emailField.addEventListener('blur', validateEmail); renderLocalOptions(); updateDataFieldsForRole();
  next.addEventListener('click', () => { if (step === 1) { setStep(2); return; } const role = selectedRole(); const active = root.querySelector('#fieldNewActivo').value === 'SI'; const name = root.querySelector('#fieldNewNombreCompleto').value.trim(); const local = selectedLocals(); const requiredFieldsValid = validateRequiredFields(); const pinsValid = validatePins(true, true, true); const emailValid = validateEmail(); if (!requiredFieldsValid || !pinsValid || !emailValid) { feedback.textContent = !requiredFieldsValid ? 'Completa los campos obligatorios marcados con * antes de continuar.' : !pinsValid ? 'Confirma el PIN y revisa que ambos campos coincidan antes de continuar.' : 'Ingresa un correo electrónico válido antes de continuar.'; feedback.classList.remove('hidden'); return; } feedback.classList.add('hidden'); root.querySelector('#newUserSummary').textContent = `${name} · ${role} · ${local.join(', ') || 'Todos los locales'} · ${active ? 'Activo' : 'Inactivo'}`; setStep(3); });
  back.addEventListener('click', () => setStep(step - 1));
  form.addEventListener('submit', async (event) => { event.preventDefault(); const pin = pinField.value.trim(); const pinsValid = validatePins(true, true, true); const emailValid = validateEmail(); if (!pinsValid || !emailValid) { feedback.textContent = !pinsValid ? 'Confirma el PIN y revisa que ambos campos coincidan antes de crear el usuario.' : 'Ingresa un correo electrónico válido.'; feedback.classList.remove('hidden'); return; }
...
function createSelectGroup(id, label, options, selectClassName = '', help = '') {
  return `
    <label class="grid gap-sm">
      <span class="text-sm font-black uppercase tracking-[0.16em] text-neutral-muted">${label}</span>
      <select id="field${id.charAt(0).toUpperCase()}${id.slice(1)}" class="min-h-[52px] rounded-2xl border border-neutral-charcoal/10 bg-white/90 px-lg py-md text-base font-semibold text-neutral-charcoal focus:border-brand-bun focus:outline-none focus:ring-2 focus:ring-brand-bun/30 ${selectClassName}">
...
function createUserTable(users, openEditModal, openInactiveActionModal) {
  ...
  users.forEach((user) => {
    ...
    row.className = 'border-t border-neutral-charcoal/8';
    row.innerHTML = `
      <td class="px-lg py-lg align-middle">
        <div class="text-base font-black text-neutral-charcoal">${escapeHtml(user.nombreCompleto)}</div>
      </td>
      <td class="px-lg py-lg align-middle text-sm font-bold text-neutral-charcoal">${escapeHtml(user.usuarioLogin || 'Sin usuario')}</td>
      <td class="px-lg py-lg align-middle">
      ...
      <td class="px-lg py-lg align-middle text-sm font-bold text-neutral-charcoal">${escapeHtml(user.local || 'Sin local')}</td>
      <td class="px-lg py-lg align-middle text-sm font-bold text-neutral-charcoal">${escapeHtml(user.cargo || 'Sin cargo')}</td>
      <td class="px-lg py-lg align-middle">
      ...
      <td class="px-lg py-lg align-middle"><div class="flex flex-wrap gap-sm">${user.activo
        ...
        : '<button type="button" class="reactivate-user icon-action-tooltip grid size-11 place-items-center rounded-full bg-brand-lettuce text-2xl font-black text-neutral-cream" data-tooltip="Reactivar usuario" aria-label="Reactivar usuario">+</button><button type="button" class="delete-user icon-action-tooltip grid size-11 place-items-center rounded-full bg-brand-ketchup text-2xl font-black text-neutral-cream" data-tooltip="Eliminar permanentemente" aria-label="Eliminar permanentemente">×</button>'
      }</div></td>
    `;
