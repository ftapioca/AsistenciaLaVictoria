...
function createNewUserModal() {
  ...
  const setFieldError = (field, invalid) => { field.classList.toggle('border-brand-ketchup', invalid); field.classList.toggle('bg-brand-ketchup/10', invalid); };
  const validatePins = (shake = false, requireConfirmation = false, showFeedback = false) => { const pin = pinField.value.trim(); const confirmPin = confirmPinField.value.trim(); const invalid = Boolean(confirmPin) && pin !== confirmPin; const container = confirmPinField.closest('div'); setFieldError(confirmPinField, invalid && showFeedback); pinFeedback.classList.toggle('hidden', !invalid || !showFeedback); container.classList.remove('animate-field-heartbeat'); if (invalid && shake && showFeedback) { container.classList.remove('animate-field-shake'); void container.offsetWidth; container.classList.add('animate-field-shake', 'animate-field-heartbeat'); } return !invalid && (!requireConfirmation || Boolean(confirmPin)); };
