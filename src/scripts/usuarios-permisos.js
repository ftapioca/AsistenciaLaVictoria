...
function createNewUserModal() {
  ...
  const pinField = root.querySelector('#fieldCreatePin'); const confirmPinField = root.querySelector('#fieldCreateConfirmPin'); const pinFeedback = root.querySelector('#newUserPinFeedback'); const emailField = root.querySelector('#fieldNewEmail'); const emailFeedback = root.querySelector('#newUserEmailFeedback');
  ...
  const validatePins = (shake = false, requireConfirmation = false, showFeedback = false) => { const pin = pinField.value.trim(); const confirmPin = confirmPinField.value.trim(); const invalid = Boolean(confirmPin) && pin !== confirmPin; setFieldError(confirmPinField, invalid && showFeedback); pinFeedback.classList.toggle('hidden', !invalid || !showFeedback); confirmPinField.classList.remove('animate-pin-alert'); if (invalid && shake && showFeedback) { void confirmPinField.offsetWidth; confirmPinField.classList.add('animate-pin-alert'); } return !invalid && (!requireConfirmation || Boolean(confirmPin)); };
