...
function createNewUserModal() {
  ...
  const updateRequirements = () => { const role = root.querySelector('#fieldNewRol').value; const requiresLogin = role === 'Administrador' || role === 'Supervisor'; const requiresLocal = role !== 'Administrador'; root.querySelector('#newUserRequirements').textContent = `Para ${role.toLowerCase()} activo: nombre y PIN obligatorios.${requiresLogin ? ' Usuario login obligatorio.' : ''}${requiresLocal ? ' Debe tener al menos un local asignado.' : ''}`; };
