...
function renderCollaborators(container, consulta) {
  ...
  exportActions.className = 'flex flex-col gap-md md:flex-row md:items-center md:justify-between';

  const exportCopy = document.createElement('p');
  exportCopy.className = 'text-sm font-semibold leading-7 text-neutral-charcoal/72';
  exportCopy.textContent = 'Completa descuentos y consumos si corresponde. El ZIP incluirá un `.xlsx` por colaborador del local seleccionado.';

  const exportButton = createButton('Exportar ZIP del local', {
    variant: 'primary',
  ...
  consulta.colaboradores.forEach((collaborator) => {
    ...
    card.className = 'rounded-3xl border border-neutral-charcoal/10 bg-white/88 p-xl shadow-brand';

    const head = document.createElement('div');
    head.className = 'grid gap-md lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)] lg:items-start';

    const identity = document.createElement('div');
    identity.className = 'grid gap-sm';
    identity.innerHTML = `
      <p class="text-xs font-black uppercase tracking-[0.18em] text-neutral-muted">Colaborador</p>
      <h3 class="text-2xl font-black tracking-[-0.03em] text-neutral-charcoal">${collaborator.colaborador}</h3>
      <p class="text-sm font-semibold text-neutral-charcoal/72">${collaborator.local} · ${consulta.periodoLabel}</p>
    `;

    const totalGrid = document.createElement('div');
    totalGrid.className = 'grid gap-sm sm:grid-cols-2 xl:grid-cols-4';
    totalGrid.append(
      ...
      createDataCell(`Trabajado: ${formatCurrency(collaborator.totalDiasTrabajados)}`),
      createDataCell(`Horas normales: ${formatCurrency(collaborator.totalHorasNormales)}`),
      createDataCell(`Horas extra: ${formatCurrency(collaborator.totalHorasExtras + collaborator.totalHorasExtrasFeriado)}`),
      createDataCell(`Total actual: ${formatCurrency(collaborator.totalPagar)}`, 'border-brand-bun/18 bg-brand-cheese/18'),
    );

    head.append(identity, totalGrid);
    card.appendChild(head);

    const inputsGrid = document.createElement('div');
    inputsGrid.className = 'mt-xl grid gap-md md:grid-cols-2 xl:grid-cols-4';
    ...
    footer.className = 'mt-lg grid gap-sm sm:grid-cols-3';

    const subtotalBox = createDataCell(`Subtotal: ${formatCurrency(subtotal)}`, 'font-black');
    const adjustmentsBox = createDataCell(
      `Ajustes: ${formatCurrency(-Number(currentInputState.descuento || 0) - Number(currentInputState.consumo || 0))}`,
      'font-black',
    );
    const totalBox = createDataCell(`${buildFinalRowLabel(consulta.periodo)}: ${formatCurrency(finalTotal())}`, 'border-brand-lettuce/25 bg-brand-lettuce/10 font-black text-brand-lettuce');

    const refreshFooter = () => {
      adjustmentsBox.textContent = `Ajustes: ${formatCurrency(-Number(currentInputState.descuento || 0) - Number(currentInputState.consumo || 0))}`;
      totalBox.textContent = `${buildFinalRowLabel(consulta.periodo)}: ${formatCurrency(finalTotal())}`;
    };
    ...
    footer.append(subtotalBox, adjustmentsBox, totalBox);
    card.appendChild(footer);

    const detailTableWrap = document.createElement('div');
    detailTableWrap.className = 'mt-xl overflow-x-auto rounded-2xl border border-neutral-charcoal/10';
    ...
    card.appendChild(detailTableWrap);
