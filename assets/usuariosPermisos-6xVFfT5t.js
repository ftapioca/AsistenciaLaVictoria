import{t as e}from"./Button-BKVxQ0e5.js";import{t}from"./LoadingOverlay-CDlm_heC.js";import"./auth-Dnlybq2p.js";import{t as n}from"./Card-BZN8aCsO.js";import{t as r}from"./PageHero-t3uoz-lT.js";import{t as i}from"./StatGrid-BHwzAMfO.js";import{t as a}from"./PageSkeletons-DIYJpQOJ.js";import{t as o}from"./Toast-CjbzkbGg.js";var s=e=>document.getElementById(e),c=[`Administrador`,`Supervisor`,`Colaborador`],l=[{key:`puede_ingresar_panel_admin`,label:`Panel administrativo`},{key:`puede_ver_mis_turnos`,label:`Ver mis turnos`},{key:`puede_programar_turnos`,label:`Programar turnos`},{key:`puede_ver_turnos_abiertos`,label:`Ver turnos abiertos`},{key:`puede_registrar_asistencia_admin`,label:`Registrar asistencia admin`},{key:`puede_ver_colaboradores_local`,label:`Ver colaboradores por local`},{key:`puede_importar_ventas`,label:`Importar ventas`},{key:`puede_ver_pagos`,label:`Ver pagos`},{key:`puede_gestionar_plantillas_turnos`,label:`Gestionar plantillas`},{key:`puede_copiar_semanas`,label:`Copiar semanas`},{key:`puede_eliminar_turnos`,label:`Eliminar turnos`}],u={session:null,users:[],roles:[],search:``,sortKey:`rol`,sortDirection:`asc`,duplicateGroups:[],editingPermissions:!1,draftPermissions:null},d=t(`Procesando...`);document.body.appendChild(d.element),d.setLoading(!0,`Validando sesión...`);var f=o();document.body.appendChild(f.element);function p(e){let t=new URL(e,window.location.href),n=window.APP_CONFIG&&window.APP_CONFIG.ENVIRONMENT;return n&&t.searchParams.set(`env`,n),t.toString()}function m(){return new Promise(e=>requestAnimationFrame(e))}function h(e){return String(e||``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#039;`)}function g(e){return String(e||``).normalize(`NFD`).replace(/[\u0300-\u036f]/g,``).trim().toLowerCase()}function _(e){return String(e||``).split(/[;,|/]+/).map(e=>e.trim()).filter(Boolean)}function v(){let e=u.users.filter(e=>e.activo).length,t=[...new Set(u.users.flatMap(e=>_(e.local)).filter(Boolean))].length,n=i([{label:`Usuarios`,value:String(u.users.length),detail:`${e} activos en la hoja Usuarios.`},{label:`Locales`,value:String(t||2),detail:`Usuarios ordenados por jerarquía y nombre.`},{label:`Entorno`,value:(window.APP_CONFIG&&window.APP_CONFIG.ENVIRONMENT||`prod`).toUpperCase(),detail:`Cada edición impacta el entorno activo.`}],{tone:`dark`});return n.id=`summaryHighlights`,n}function y(){let e=s(`summaryHighlights`),t=v();e&&e.parentNode&&e.parentNode.replaceChild(t,e)}function b(){let e=document.createElement(`div`);e.id=`editUserModal`,e.className=`fixed inset-0 z-[140] hidden items-center justify-center bg-neutral-charcoal/68 px-lg py-lg backdrop-blur`,e.innerHTML=`
  ...
  `,f.append(u,h);let g=document.createElement(`div`);g.id=`usersTable`,g.className=`mt-xl overflow-hidden rounded-2xl border border-neutral-charcoal/10 bg-white/84`,l.append(f,g);let _=n({eyebrow:`Limpieza controlada`,title:`Duplicados de usuarios`,body:`Revisa cada coincidencia, elige el registro canónico y confirma antes de consolidar. No se eliminan filas: los duplicados quedan inactivos y trazables.`,className:`rounded-3xl md:p-2xl`}),y=e(`Revisar duplicados`,{variant:`secondary`,className:`mt-xl bg-white/88 text-neutral-charcoal hover:bg-white`});y.id=`btnAuditDuplicates`;let b=document.createElement(`div`);b.id=`duplicatesRoot`,b.className=`mt-xl grid gap-lg`,_.append(y,b);let x=n({eyebrow:`Permisos`,title:`Matriz por tipo de usuario`,body:`Comparación compacta por permiso y por rol. El orden fijo es Administrador, Supervisor, Colaborador.`,className:`rounded-3xl md:p-2xl`}),S=document.createElement(`div`);S.id=`permissionsMatrix`,S.className=`mt-xl overflow-hidden rounded-3xl border border-neutral-charcoal/10 bg-white/88 shadow-brand-sm`,x.appendChild(S),i.append(c,l,_,x),t.replaceChildren(i)}function w(){let e={};return u.roles.forEach(t=>{e[t.role]=t.permissions||{}}),e}function T(){let e=g(u.search),t=new Map(c.map((e,t)=>[e,t]));return u.users.filter(t=>e?g([t.nombreCompleto,t.usuarioLogin,t.local,t.cargo,t.rol,t.email,t.telefono].join(` `)).includes(e):!0).sort((e,n)=>{let r=(e,t)=>String(e||``).localeCompare(String(t||``),`es`),i=()=>(t.get(e.rol)??c.length)-(t.get(n.rol)??c.length),a={nombreCompleto:r(e.nombreCompleto,n.nombreCompleto),usuarioLogin:r(e.usuarioLogin,n.usuarioLogin),rol:i(),local:r(e.local,n.local),cargo:r(e.cargo,n.cargo),activo:Number(!!e.activo)-Number(!!n.activo)}[u.sortKey]??0;if(a)return a*(u.sortDirection===`asc`?1:-1);if(u.sortKey!==`rol`){let e=i();if(e)return e}return r(e.nombreCompleto,n.nombreCompleto)})}function E(e,t){let n=[{key:`nombreCompleto`,label:`Nombre`},{key:`usuarioLogin`,label:`Usuario`},{key:`rol`,label:`Rol`},{key:`local`,label:`Local`},{key:`cargo`,label:`Cargo`},{key:`activo`,label:`Estado`}],r=document.createElement(`div`);r.className=`overflow-x-auto`,r.innerHTML=`
  ...
  `,r.querySelectorAll(`[data-sort-key]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.sortKey;u.sortKey===t?u.sortDirection=u.sortDirection===`asc`?`desc`:`asc`:(u.sortKey=t,u.sortDirection=`asc`),D()})});let i=r.querySelector(`tbody`);return e.forEach(e=>{let n=document.createElement(`tr`);n.className=`border-t border-neutral-charcoal/8 align-top`,n.innerHTML=`
    ...
    `,n.querySelector(`.edit-user`).addEventListener(`click`,()=>t(e)),i.appendChild(n)}),r}function D(){let e=s(`usersTable`),t=window.__lvEditUserModal,n=T();if(s(`usersMeta`).textContent=`${n.length} usuario(s) visibles. Orden: rol, local y nombre.`,e.innerHTML=``,!n.length){e.innerHTML=`<div class="px-lg py-xl text-sm font-bold text-neutral-muted">No hay usuarios que coincidan con la búsqueda.</div>`;return}e.appendChild(E(n,t.open))}async function O(){let e=await window.LVAuth.apiGet({accion:`AuditarDuplicadosUsuariosAdmin`});if(e.status!==`SUCCESS`)throw Error(e.mensaje||`No se pudieron revisar los duplicados.`);return Array.isArray(e.groups)?e.groups:[]}function k(){let e=s(`duplicatesRoot`);if(e.innerHTML=``,!u.duplicateGroups.length){e.innerHTML=`<div class="rounded-2xl border border-brand-lettuce/20 bg-brand-lettuce/10 px-lg py-lg text-sm font-bold text-brand-lettuce">No se detectaron grupos duplicados.</div>`;return}u.duplicateGroups.forEach(t=>{let n=document.createElement(`section`);n.className=`rounded-3xl border border-brand-cheese/30 bg-brand-cheese/10 p-lg`,n.innerHTML=`
      <div class="flex flex-col gap-sm md:flex-row md:items-start md:justify-between">
        <div>
          <p class="text-sm font-black text-neutral-charcoal">${t.candidates.length} registros · ${h(t.role)}</p>
          <p class="mt-xs text-sm font-bold text-neutral-muted">${t.matchType===`usuario_login`?`Coincidencia por usuario login.`:`Coincidencia por nombre y rol sin usuario login. Revísala antes de consolidar.`}</p>
        </div>
      </div>
      <label class="mt-lg grid gap-sm text-sm font-black text-neutral-charcoal">
        ...
        <select class="duplicate-canonical min-h-[50px] rounded-2xl border border-neutral-charcoal/10 bg-white px-lg py-md font-bold">
          ${t.candidates.map(e=>`<option value="${h(e.idUsuario)}" ${e.recommended?`selected`:``}>${h(e.nombreCompleto)} · ${h(e.idUsuario)} · ${e.activo?`Activo`:`Inactivo`} · ${h(e.local||`Sin local`)}</option>`).join(``)}
        </select>
      </label>
    ...
    `;let r=n.querySelector(`.duplicate-confirm`),i=n.querySelector(`.consolidate-duplicate`);r.addEventListener(`change`,()=>{i.disabled=!r.checked}),i.addEventListener(`click`,async()=>{d.setLoading(!0,`Consolidando usuarios...`,`Estamos preservando el registro elegido y dejando trazabilidad en los duplicados.`),await m();try{let e=await window.LVAuth.apiPost({accion:`ConsolidarDuplicadosUsuariosAdmin`,groupKey:t.groupKey,canonicalId:n.querySelector(`.duplicate-canonical`).value,confirmar:`SI`});if(e.status!==`SUCCESS`)throw Error(e.mensaje||`No se pudo consolidar el grupo.`);await j(),y(),D(),u.duplicateGroups=await O(),k(),f.show(`success`,`Grupo consolidado en ${e.canonicalId}.`)}catch(e){f.show(`error`,e.message||`No se pudo consolidar el grupo.`)}finally{d.setLoading(!1)}}),e.appendChild(n)})}function A(){let e=s(`permissionsMatrix`),t=u.editingPermissions&&u.draftPermissions?u.draftPermissions:w();if(e.innerHTML=`
    <div class="relative">
  ...
  `,!u.editingPermissions){e.querySelector(`#btnEditPermissionsMatrix`).addEventListener(`click`,()=>{u.editingPermissions=!0,u.draftPermissions=JSON.parse(JSON.stringify(w())),A()});return}e.querySelectorAll(`.permission-checkbox`).forEach(e=>{e.addEventListener(`change`,()=>{let t=e.dataset.role,n=e.dataset.permission;u.draftPermissions[t]||(u.draftPermissions[t]={}),u.draftPermissions[t][n]=e.checked,A()})}),e.querySelector(`#btnCancelPermissionsEdit`).addEventListener(`click`,()=>{u.editingPermissions=!1,u.draftPermissions=null,A()}),e.querySelector(`#btnSavePermissionsMatrix`).addEventListener(`click`,async()=>{let t=e.querySelector(`#btnSavePermissionsMatrix`),n=e.querySelector(`#btnCancelPermissionsEdit`),r=e.querySelector(`#permissionsSaveFeedback`);t.disabled=!0,n&&(n.disabled=!0),t.textContent=`Guardando...`,r.classList.add(`hidden`),d.setLoading(!0,`Guardando matriz de permisos...`,`Estamos aplicando los permisos definidos para cada rol.`),await m();try{for(let e of c){let t={accion:`ActualizarPermisosRolAdmin`,role:e};l.forEach(n=>{t[n.key]=u.draftPermissions[e]&&u.draftPermissions[e][n.key]?`SI`:`NO`});let n=await window.LVAuth.apiPost(t);if(n.status!==`SUCCESS`)throw Error(n.mensaje||`No se pudieron guardar los permisos de ${e}.`);u.roles=u.roles.map(t=>t.role===e?n.role:t)}u.editingPermissions=!1,u.draftPermissions=null,f.show(`success`,`Matriz de permisos actualizada.`),A()}catch(e){r.textContent=e.message||`No se pudieron guardar los permisos.`,r.classList.remove(`hidden`),f.show(`error`,e.message||`No se pudieron guardar los permisos.`)}finally{d.setLoading(!1),t.disabled=!1,n&&(n.disabled=!1),t.textContent=`Guardar permisos`}})}async function j(){d.setLoading(!0,`Cargando usuarios y permisos...`),await m();let e=await window.LVAuth.apiGet({accion:`BootstrapGestionUsuarios`});if(e.status!==`SUCCESS`)throw Error(e.mensaje||`No se pudieron cargar los usuarios.`);u.users=Array.isArray(e.users)?e.users:[],u.roles=Array.isArray(e.roles)?e.roles:[]}function M(){s(`userSearchInput`).addEventListener(`input`,e=>{u.search=e.target.value||``,D()}),s(`btnAuditDuplicates`).addEventListener(`click`,async()=>{d.setLoading(!0,`Revisando duplicados...`,`Estamos comparando identidades sin modificar registros.`),await m();try{u.duplicateGroups=await O(),k()}catch(e){f.show(`error`,e.message||`No se pudieron revisar los duplicados.`)}finally{d.setLoading(!1)}})}async function N(){if(s(`app`).innerHTML=``,a({mountNode:s(`app`),variant:`table`}),d.setLoading(!0,`Validando sesión...`,`Estamos comprobando el acceso administrativo y preparando la carga de usuarios y permisos.`),u.session=await window.LVAuth.protectPage([window.LVAuth.roles.ADMINISTRADOR]),u.session){C(),M(),window.__lvEditUserModal=b(),s(`sessionStatus`).textContent=`${u.session.displayName||`Administrador`} · ${u.session.role}`;try{await j(),y(),D(),A()}finally{d.setLoading(!1)}}}N().catch(e=>{d.setLoading(!1),f.show(`error`,e.message||`No se pudo cargar la gestión de usuarios.`)});