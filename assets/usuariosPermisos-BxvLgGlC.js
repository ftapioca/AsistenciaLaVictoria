import{t as e}from"./Button-C6BQ9pjW.js";import{t}from"./LoadingOverlay-BNfztysg.js";import"./auth-Dnlybq2p.js";import{t as n}from"./Card-x-sWGlbv.js";import{t as r}from"./PageHero-CxRrt6Y1.js";import{t as i}from"./StatGrid-BU6bEjkC.js";import{t as a}from"./PageSkeletons-DIYJpQOJ.js";import{t as o}from"./Toast-BVwJ2Bhe.js";var s=e=>document.getElementById(e),c=[`Administrador`,`Supervisor`,`Colaborador`],l=[{key:`puede_ingresar_panel_admin`,label:`Panel administrativo`},{key:`puede_ver_mis_turnos`,label:`Ver mis turnos`},{key:`puede_programar_turnos`,label:`Programar turnos`},{key:`puede_ver_turnos_abiertos`,label:`Ver turnos abiertos`},{key:`puede_registrar_asistencia_admin`,label:`Registrar asistencia admin`},{key:`puede_ver_colaboradores_local`,label:`Ver colaboradores por local`},{key:`puede_importar_ventas`,label:`Importar ventas`},{key:`puede_ver_pagos`,label:`Ver pagos`},{key:`puede_gestionar_plantillas_turnos`,label:`Gestionar plantillas`},{key:`puede_copiar_semanas`,label:`Copiar semanas`},{key:`puede_eliminar_turnos`,label:`Eliminar turnos`}],u={session:null,users:[],roles:[],search:``,editingPermissions:!1,draftPermissions:null},d=t(`Procesando...`);document.body.appendChild(d.element),d.setLoading(!0,`Validando sesión...`);var f=o();document.body.appendChild(f.element);function p(e){let t=new URL(e,window.location.href),n=window.APP_CONFIG&&window.APP_CONFIG.ENVIRONMENT;return n&&t.searchParams.set(`env`,n),t.toString()}function m(){return new Promise(e=>requestAnimationFrame(e))}function h(e){return String(e||``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#039;`)}function g(e){return String(e||``).normalize(`NFD`).replace(/[\u0300-\u036f]/g,``).trim().toLowerCase()}function _(e){return String(e||``).split(/[;,|/]+/).map(e=>e.trim()).filter(Boolean)}function v(){let e=u.users.filter(e=>e.activo).length,t=[...new Set(u.users.flatMap(e=>_(e.local)).filter(Boolean))].length,n=i([{label:`Usuarios`,value:String(u.users.length),detail:`${e} activos en la hoja Usuarios.`},{label:`Locales`,value:String(t||2),detail:`La vista separa operación por local y administración global.`},{label:`Entorno`,value:(window.APP_CONFIG&&window.APP_CONFIG.ENVIRONMENT||`prod`).toUpperCase(),detail:`Cada edición impacta el entorno activo.`}],{tone:`dark`});return n.id=`summaryHighlights`,n}function y(){let e=s(`summaryHighlights`),t=v();e&&e.parentNode&&e.parentNode.replaceChild(t,e)}function b(){let e=document.createElement(`div`);e.id=`editUserModal`,e.className=`fixed inset-0 z-[140] hidden items-center justify-center bg-neutral-charcoal/68 px-lg py-lg backdrop-blur`,e.innerHTML=`
  ...
  `,l.appendChild(u);let f=n({eyebrow:`Usuarios`,title:`Lista de usuarios`,body:`Administradores, supervisores y colaboradores se muestran en una sola tabla, ordenados por jerarquía y local asignado.`,className:`rounded-3xl md:p-2xl`}),h=document.createElement(`div`);h.id=`usersMeta`,h.className=`mt-lg text-sm font-bold text-neutral-muted`;let g=document.createElement(`div`);g.id=`usersTable`,g.className=`mt-xl overflow-hidden rounded-2xl border border-neutral-charcoal/10 bg-white/84`,f.append(h,g);let _=n({eyebrow:`Permisos`,title:`Matriz por tipo de usuario`,body:`Comparación compacta por permiso y por rol. El orden fijo es Administrador, Supervisor, Colaborador.`,className:`rounded-3xl md:p-2xl`}),y=document.createElement(`div`);y.id=`permissionsMatrix`,y.className=`mt-xl overflow-hidden rounded-3xl border border-neutral-charcoal/10 bg-white/88 shadow-brand-sm`,_.appendChild(y),i.append(c,l,f,_),t.replaceChildren(i)}function w(){let e={};return u.roles.forEach(t=>{e[t.role]=t.permissions||{}}),e}function T(){let e=g(u.search),t=new Map(c.map((e,t)=>[e,t]));return u.users.filter(t=>e?g([t.nombreCompleto,t.usuarioLogin,t.local,t.cargo,t.rol,t.email,t.telefono].join(` `)).includes(e):!0).sort((e,n)=>(t.get(e.rol)??c.length)-(t.get(n.rol)??c.length)||String(e.local||``).localeCompare(String(n.local||``),`es`)||String(e.nombreCompleto||``).localeCompare(String(n.nombreCompleto||``),`es`))}function E(e,t){let n=document.createElement(`div`);n.className=`overflow-x-auto`,n.innerHTML=`
