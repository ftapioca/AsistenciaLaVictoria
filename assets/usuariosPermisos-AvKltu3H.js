import{n as e,t}from"./Card-CaVi7Iaf.js";import{t as n}from"./LoadingOverlay-0oRYag-G.js";import"./auth-D21mG5Br.js";import{t as r}from"./PageHero-Dz09WTDW.js";import{t as i}from"./StatGrid-s8SsHm4L.js";import{t as a}from"./PageSkeletons-DIYJpQOJ.js";import{t as o}from"./Toast-DynmMtmN.js";var s=e=>document.getElementById(e),c=980,l=[`Administrador`,`Supervisor`,`Colaborador`],u=[{key:`puede_ingresar_panel_admin`,label:`Panel administrativo`},{key:`puede_ver_mis_turnos`,label:`Ver mis turnos`},{key:`puede_programar_turnos`,label:`Programar turnos`},{key:`puede_ver_turnos_abiertos`,label:`Ver turnos abiertos`},{key:`puede_registrar_asistencia_admin`,label:`Registrar asistencia admin`},{key:`puede_ver_colaboradores_local`,label:`Ver colaboradores por local`},{key:`puede_importar_ventas`,label:`Importar ventas`},{key:`puede_ver_pagos`,label:`Ver pagos`},{key:`puede_gestionar_plantillas_turnos`,label:`Gestionar plantillas`},{key:`puede_copiar_semanas`,label:`Copiar semanas`},{key:`puede_eliminar_turnos`,label:`Eliminar turnos`}],d={session:null,users:[],roles:[],search:``},f=n(`Procesando...`);document.body.appendChild(f.element),f.setLoading(!0,`Validando sesión...`);var p=o();document.body.appendChild(p.element);function m(e){let t=new URL(e,window.location.href),n=window.APP_CONFIG&&window.APP_CONFIG.ENVIRONMENT;return n&&t.searchParams.set(`env`,n),t.toString()}function h(){return new Promise(e=>requestAnimationFrame(e))}function g(e){return String(e||``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#039;`)}function _(e){return String(e||``).normalize(`NFD`).replace(/[\u0300-\u036f]/g,``).trim().toLowerCase()}function v(e){return String(e||``).split(/[;,|/]+/).map(e=>e.trim()).filter(Boolean)}function y(){return window.innerWidth<=c}function b(){let e=d.users.filter(e=>e.activo).length,t=[...new Set(d.users.flatMap(e=>v(e.local)).filter(Boolean))].length,n=i([{label:`Usuarios`,value:String(d.users.length),detail:`${e} activos en la hoja Usuarios.`},{label:`Locales`,value:String(t||2),detail:`La vista separa operación por local y administración global.`},{label:`Entorno`,value:(window.APP_CONFIG&&window.APP_CONFIG.ENVIRONMENT||`prod`).toUpperCase(),detail:`Cada edición impacta el entorno activo.`}],{tone:`dark`});return n.id=`summaryHighlights`,n}function x(){let e=s(`summaryHighlights`),t=b();e&&e.parentNode&&e.parentNode.replaceChild(t,e)}function S({id:e,title:n,subtitle:r,badgeText:i=``,open:a=!1}){let o=t({className:`overflow-hidden rounded-3xl p-0`}),s=document.createElement(`section`);s.dataset.accordionId=e;let c=document.createElement(`button`);c.type=`button`,c.className=`flex w-full items-center justify-between gap-lg px-xl py-xl text-left`,c.setAttribute(`aria-expanded`,a?`true`:`false`),c.setAttribute(`aria-controls`,`${e}-panel`);let l=document.createElement(`div`);l.className=`min-w-0`,l.innerHTML=`
  ...
  `,document.body.appendChild(e);let t=e.querySelector(`#editUserForm`),n=e.querySelector(`[data-modal-backdrop]`),r=e.querySelector(`#editUserTitle`),i=e.querySelector(`#editUserSubtitle`),a=e.querySelector(`#btnSaveEditUser`),o=``;function s(){e.classList.add(`hidden`),e.classList.remove(`grid`),o=``}function c(e){a.disabled=e}function u(t,n){let r=e.querySelector(`#${t}`);r&&(r.value=n||``)}function f(t){o=t.idUsuario,r.textContent=t.nombreCompleto||`Usuario`,i.textContent=`${t.local||`Sin local`} · ${t.rol||`Sin rol`}`,u(`fieldIdUsuario`,t.idUsuario),u(`fieldNombreCompleto`,t.nombreCompleto),u(`fieldUsuarioLogin`,t.usuarioLogin),u(`fieldPin`,t.pin),u(`fieldRol`,t.rol),u(`fieldLocal`,t.local),u(`fieldCargo`,t.cargo),u(`fieldActivo`,t.activo?`SI`:`NO`),u(`fieldEmail`,t.email),u(`fieldTelefono`,t.telefono),u(`fieldFechaCreacion`,t.fechaCreacion),u(`fieldObservaciones`,t.observaciones),e.classList.remove(`hidden`),e.classList.add(`grid`)}async function m(t){t.preventDefault(),c(!0);try{let t=await window.LVAuth.apiPost({accion:`ActualizarUsuarioAdmin`,idUsuario:o,nombreCompleto:e.querySelector(`#fieldNombreCompleto`).value.trim(),usuarioLogin:e.querySelector(`#fieldUsuarioLogin`).value.trim(),pin:e.querySelector(`#fieldPin`).value.trim(),rol:e.querySelector(`#fieldRol`).value.trim(),local:e.querySelector(`#fieldLocal`).value.trim(),cargo:e.querySelector(`#fieldCargo`).value.trim(),activo:e.querySelector(`#fieldActivo`).value.trim(),email:e.querySelector(`#fieldEmail`).value.trim(),telefono:e.querySelector(`#fieldTelefono`).value.trim(),fechaCreacion:e.querySelector(`#fieldFechaCreacion`).value.trim(),observaciones:e.querySelector(`#fieldObservaciones`).value.trim()});if(t.status!==`SUCCESS`)throw Error(t.mensaje||`No se pudo actualizar el usuario.`);d.users=d.users.map(e=>e.idUsuario===t.user.idUsuario?t.user:e),x(),j(),p.show(`success`,`${t.user.nombreCompleto} actualizado correctamente.`),s()}catch(e){p.show(`error`,e.message||`No se pudo actualizar el usuario.`)}finally{c(!1)}}return t.addEventListener(`submit`,m),e.querySelector(`#btnCloseEditUser`).addEventListener(`click`,s),e.querySelector(`#btnCancelEditUser`).addEventListener(`click`,s),n.addEventListener(`click`,s),{open:f,close:s}}function w(e,t,n=!1){return`
    <label class="grid gap-sm">
      <span class="text-sm font-black uppercase tracking-[0.16em] text-neutral-muted">${t}</span>
      <input id="field${e.charAt(0).toUpperCase()}${e.slice(1)}" ${n?`disabled`:``} class="min-h-[52px] rounded-2xl border border-neutral-charcoal/10 bg-white/90 px-lg py-md text-base font-semibold text-neutral-charcoal placeholder:text-neutral-muted/70 focus:border-brand-bun focus:outline-none focus:ring-2 focus:ring-brand-bun/30 ${n?`opacity-60`:``}">
    </label>
