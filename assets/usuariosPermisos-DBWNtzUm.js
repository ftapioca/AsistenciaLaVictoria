import{n as e,t}from"./Card-sJzzOyhq.js";import{t as n}from"./LoadingOverlay-Bffw7pAr.js";import"./auth-D21mG5Br.js";import{t as r}from"./PageHero-Cy4jFz-F.js";import{t as i}from"./StatGrid-tVaODvnW.js";import{t as a}from"./PageSkeletons-DIYJpQOJ.js";import{t as o}from"./Toast-BC-U7wuX.js";var s=e=>document.getElementById(e),c=980,l=[`Administrador`,`Supervisor`,`Colaborador`],u=[{key:`puede_ingresar_panel_admin`,label:`Panel administrativo`},{key:`puede_ver_mis_turnos`,label:`Ver mis turnos`},{key:`puede_programar_turnos`,label:`Programar turnos`},{key:`puede_ver_turnos_abiertos`,label:`Ver turnos abiertos`},{key:`puede_registrar_asistencia_admin`,label:`Registrar asistencia admin`},{key:`puede_ver_colaboradores_local`,label:`Ver colaboradores por local`},{key:`puede_importar_ventas`,label:`Importar ventas`},{key:`puede_ver_pagos`,label:`Ver pagos`},{key:`puede_gestionar_plantillas_turnos`,label:`Gestionar plantillas`},{key:`puede_copiar_semanas`,label:`Copiar semanas`},{key:`puede_eliminar_turnos`,label:`Eliminar turnos`}],d={session:null,users:[],roles:[],search:``},f=n(`Procesando...`);document.body.appendChild(f.element),f.setLoading(!0,`Validando sesión...`);var p=o();document.body.appendChild(p.element);function m(e){let t=new URL(e,window.location.href),n=window.APP_CONFIG&&window.APP_CONFIG.ENVIRONMENT;return n&&t.searchParams.set(`env`,n),t.toString()}function h(){return new Promise(e=>requestAnimationFrame(e))}function g(e){return String(e||``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#039;`)}function _(e){return String(e||``).normalize(`NFD`).replace(/[\u0300-\u036f]/g,``).trim().toLowerCase()}function v(e){return String(e||``).split(/[;,|/]+/).map(e=>e.trim()).filter(Boolean)}function y(){return window.innerWidth<=c}function b(){let e=d.users.filter(e=>e.activo).length,t=[...new Set(d.users.flatMap(e=>v(e.local)).filter(Boolean))].length,n=i([{label:`Usuarios`,value:String(d.users.length),detail:`${e} activos en la hoja Usuarios.`},{label:`Locales`,value:String(t||2),detail:`La vista separa operación por local y administración global.`},{label:`Entorno`,value:(window.APP_CONFIG&&window.APP_CONFIG.ENVIRONMENT||`prod`).toUpperCase(),detail:`Cada edición impacta el entorno activo.`}],{tone:`dark`});return n.id=`summaryHighlights`,n}function x(){let e=s(`summaryHighlights`),t=b();e&&e.parentNode&&e.parentNode.replaceChild(t,e)}function S({id:e,title:n,subtitle:r,badgeText:i=``,open:a=!1}){let o=t({className:`overflow-hidden rounded-3xl p-0`}),s=document.createElement(`section`);s.dataset.accordionId=e;let c=document.createElement(`button`);c.type=`button`,c.className=`flex w-full items-center justify-between gap-lg px-xl py-xl text-left`,c.setAttribute(`aria-expanded`,a?`true`:`false`),c.setAttribute(`aria-controls`,`${e}-panel`);let l=document.createElement(`div`);l.className=`min-w-0`,l.innerHTML=`
  ...
  `,document.body.appendChild(e);let t=e.querySelector(`#editUserForm`),n=e.querySelector(`[data-modal-backdrop]`),r=e.querySelector(`#editUserTitle`),i=e.querySelector(`#editUserSubtitle`),a=e.querySelector(`#btnSaveEditUser`),o=e.querySelector(`#btnCancelEditUser`),s=e.querySelector(`#btnCloseEditUser`),c=e.querySelector(`#btnTogglePinChange`),u=e.querySelector(`#pinChangePanel`),f=e.querySelector(`#editUserFeedback`),m=``,h=!1;function g(){e.classList.add(`hidden`),e.classList.remove(`grid`),m=``,h=!1,u.classList.add(`hidden`),c.textContent=`Nuevo PIN`,v(`fieldNewPin`,``),v(`fieldConfirmNewPin`,``),y(``)}function _(e){a.disabled=e,o.disabled=e,s.disabled=e,c.disabled=e,a.textContent=e?`Guardando...`:`Guardar cambios`}function v(t,n){let r=e.querySelector(`#${t}`);r&&(r.value=n||``)}function y(e){if(!e){f.textContent=``,f.classList.add(`hidden`);return}f.textContent=e,f.classList.remove(`hidden`)}function b(t){m=t.idUsuario,r.textContent=t.nombreCompleto||`Usuario`,i.textContent=`${t.local||`Sin local`} · ${t.rol||`Sin rol`}`,v(`fieldIdUsuario`,t.idUsuario),v(`fieldNombreCompleto`,t.nombreCompleto),v(`fieldUsuarioLogin`,t.usuarioLogin),v(`fieldRol`,t.rol),v(`fieldLocal`,t.local),v(`fieldCargo`,t.cargo),v(`fieldActivo`,t.activo?`SI`:`NO`),v(`fieldEmail`,t.email),v(`fieldTelefono`,t.telefono),v(`fieldFechaCreacion`,t.fechaCreacion),v(`fieldObservaciones`,t.observaciones),v(`fieldNewPin`,``),v(`fieldConfirmNewPin`,``),y(``),h=!1,u.classList.add(`hidden`),c.textContent=`Nuevo PIN`,e.classList.remove(`hidden`),e.classList.add(`grid`)}async function S(t){t.preventDefault();let n=e.querySelector(`#fieldNewPin`).value.trim(),r=e.querySelector(`#fieldConfirmNewPin`).value.trim();if(h){if(!n){p.show(`error`,`Debes ingresar el nuevo PIN.`);return}if(n!==r){p.show(`error`,`La confirmación del nuevo PIN no coincide.`);return}}_(!0),y(`Guardando cambios del usuario...`);try{let t=await window.LVAuth.apiPost({accion:`ActualizarUsuarioAdmin`,idUsuario:m,nombreCompleto:e.querySelector(`#fieldNombreCompleto`).value.trim(),usuarioLogin:e.querySelector(`#fieldUsuarioLogin`).value.trim(),newPin:h?n:``,rol:e.querySelector(`#fieldRol`).value.trim(),local:e.querySelector(`#fieldLocal`).value.trim(),cargo:e.querySelector(`#fieldCargo`).value.trim(),activo:e.querySelector(`#fieldActivo`).value.trim(),email:e.querySelector(`#fieldEmail`).value.trim(),telefono:e.querySelector(`#fieldTelefono`).value.trim(),fechaCreacion:e.querySelector(`#fieldFechaCreacion`).value.trim(),observaciones:e.querySelector(`#fieldObservaciones`).value.trim()});if(t.status!==`SUCCESS`)throw Error(t.mensaje||`No se pudo actualizar el usuario.`);d.users=d.users.map(e=>e.idUsuario===t.user.idUsuario?t.user:e),x(),j(),p.show(`success`,`${t.user.nombreCompleto} actualizado correctamente.`),g()}catch(e){y(``),p.show(`error`,e.message||`No se pudo actualizar el usuario.`)}finally{_(!1)}}return t.addEventListener(`submit`,S),c.addEventListener(`click`,()=>{h=!h,u.classList.toggle(`hidden`,!h),c.textContent=h?`Cancelar cambio de PIN`:`Nuevo PIN`,h||(v(`fieldNewPin`,``),v(`fieldConfirmNewPin`,``))}),e.querySelector(`#btnCloseEditUser`).addEventListener(`click`,g),e.querySelector(`#btnCancelEditUser`).addEventListener(`click`,g),n.addEventListener(`click`,g),{open:b,close:g}}function w(e,t,n=!1,r=`text`){return`
    <label class="grid gap-sm">
  ...
  `,l.appendChild(u);let d=t({eyebrow:`Usuarios`,title:`Roles por persona`,body:`Separamos la operación por acordeón: cada local muestra sus propios usuarios y el bloque de administradores queda aparte.`,className:`rounded-3xl md:p-2xl`}),p=document.createElement(`div`);p.id=`usersMeta`,p.className=`mt-lg text-sm font-bold text-neutral-muted`;let g=document.createElement(`div`);g.id=`usersAccordions`,g.className=`mt-xl grid gap-lg`,d.append(p,g);let _=t({eyebrow:`Permisos`,title:`Matriz por tipo de usuario`,body:`Comparación compacta por permiso y por rol. El orden fijo es Administrador, Supervisor, Colaborador.`,className:`rounded-3xl md:p-2xl`}),v=document.createElement(`div`);v.id=`permissionsMatrix`,v.className=`mt-xl overflow-hidden rounded-3xl border border-neutral-charcoal/10 bg-white/88 shadow-brand-sm`,_.appendChild(v),i.append(c,l,d,_),n.replaceChildren(i)}function D(){let e={};return d.roles.forEach(t=>{e[t.role]=t.permissions||{}}),e}function O(){let e=_(d.search);return d.users.filter(t=>e?_([t.nombreCompleto,t.usuarioLogin,t.local,t.cargo,t.rol,t.email,t.telefono].join(` `)).includes(e):!0)}function k(){let e=O(),t=[...new Set(e.filter(e=>e.rol!==`Administrador`).flatMap(e=>v(e.local)).filter(Boolean))].sort((e,t)=>e.localeCompare(t,`es`)).map(t=>({id:`local-${_(t)}`,title:t,subtitle:`Usuarios operativos asignados a este local`,users:e.filter(e=>e.rol!==`Administrador`&&v(e.local).includes(t))}));return t.push({id:`administradores`,title:`Administradores`,subtitle:`Usuarios con control global del sistema`,users:e.filter(e=>e.rol===`Administrador`)}),t}function A(e,t){let n=document.createElement(`div`);n.className=`overflow-x-auto`,n.innerHTML=`
      ...
      <tbody>
        ${u.map(e=>`
          <tr class="border-t border-neutral-charcoal/8">
            ...
            ${l.map(n=>{let r=!!(t[n]&&t[n][e.key]);return`
                <td class="px-lg py-lg text-center">
                  <button
                    ...
                    class="permission-toggle inline-flex min-h-[42px] min-w-[42px] items-center justify-center rounded-2xl border text-2xl font-black transition-fast ${r?`border-brand-lettuce/18 bg-brand-lettuce/12 text-brand-lettuce`:`border-brand-ketchup/18 bg-brand-ketchup/10 text-brand-ketchup`}"
                  >
                    ${r?`✓`:`×`}
                  </button>
                </td>
  ...
  `,e.querySelectorAll(`.permission-toggle`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.active===`true`;e.dataset.active=t?`false`:`true`,e.textContent=t?`×`:`✓`,e.className=`permission-toggle inline-flex min-h-[42px] min-w-[42px] items-center justify-center rounded-2xl border text-2xl font-black transition-fast ${t?`border-brand-ketchup/18 bg-brand-ketchup/10 text-brand-ketchup`:`border-brand-lettuce/18 bg-brand-lettuce/12 text-brand-lettuce`}`})}),e.querySelector(`#btnSavePermissionsMatrix`).addEventListener(`click`,async()=>{let t=e.querySelector(`#btnSavePermissionsMatrix`),n=e.querySelector(`#permissionsSaveFeedback`);t.disabled=!0,t.textContent=`Guardando...`,n.textContent=`Guardando matriz de permisos...`,n.classList.remove(`hidden`);try{for(let t of l){let n={accion:`ActualizarPermisosRolAdmin`,role:t};e.querySelectorAll(`.permission-toggle[data-role="${t}"]`).forEach(e=>{n[e.dataset.permission]=e.dataset.active===`true`?`SI`:`NO`});let r=await window.LVAuth.apiPost(n);if(r.status!==`SUCCESS`)throw Error(r.mensaje||`No se pudieron guardar los permisos de ${t}.`);d.roles=d.roles.map(e=>e.role===t?r.role:e)}p.show(`success`,`Matriz de permisos actualizada.`),M()}catch(e){n.classList.add(`hidden`),p.show(`error`,e.message||`No se pudieron guardar los permisos.`)}finally{t.disabled=!1,t.textContent=`Guardar permisos`}})}async function N(){f.setLoading(!0,`Cargando usuarios y permisos...`),await h();let e=await window.LVAuth.apiGet({accion:`BootstrapGestionUsuarios`});if(e.status!==`SUCCESS`)throw Error(e.mensaje||`No se pudieron cargar los usuarios.`);d.users=Array.isArray(e.users)?e.users:[],d.roles=Array.isArray(e.roles)?e.roles:[]}function P(){s(`userSearchInput`).addEventListener(`input`,e=>{d.search=e.target.value||``,j()})}async function F(){if(s(`app`).innerHTML=``,a({mountNode:s(`app`),variant:`table`}),f.setLoading(!0,`Validando sesión...`,`Estamos comprobando el acceso administrativo y preparando la carga de usuarios y permisos.`),d.session=await window.LVAuth.protectPage([window.LVAuth.roles.ADMINISTRADOR]),d.session){E(),P(),window.__lvEditUserModal=C(),s(`sessionStatus`).textContent=`${d.session.displayName||`Administrador`} · ${d.session.role}`;try{await N(),x(),j(),M()}finally{f.setLoading(!1)}}}F().catch(e=>{f.setLoading(!1),p.show(`error`,e.message||`No se pudo cargar la gestión de usuarios.`)});